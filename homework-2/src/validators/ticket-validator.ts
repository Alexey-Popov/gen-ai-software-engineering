...
const createTicketSchema = z.object({
  ...
  category: z.nativeEnum(Category, { errorMap: () => ({ message: `category must be one of: ${Object.values(Category).join(', ')}` }) }),
  priority: z.nativeEnum(Priority, { errorMap: () => ({ message: `priority must be one of: ${Object.values(Priority).join(', ')}` }) }),
...
const updateTicketSchema = z.object({
  ...
  category: z.nativeEnum(Category, { errorMap: () => ({ message: `category must be one of: ${Object.values(Category).join(', ')}` }) }).optional(),
  priority: z.nativeEnum(Priority, { errorMap: () => ({ message: `priority must be one of: ${Object.values(Priority).join(', ')}` }) }).optional(),
  status: z.nativeEnum(Status, { errorMap: () => ({ message: `status must be one of: ${Object.values(Status).join(', ')}` }) }).optional(),
  assigned_to: z.string().optional(),
  tags: z.array(z.string()).optional(),
  metadata: metadataSchema.optional(),
